...
def remplirdataset(taille, ratio, distri=None,mu=0,sigma=0,shape=0,scale=0):
    ...
    if distri is None:    
        ...
        dataset[:,1] = np.random.pareto(1,taille)         
        dataset[:,2] = np.random.pareto(1,taille)
        dataset[:,3] = np.random.pareto(1,taille)
        dataset[:,4] = np.random.pareto(1,taille)
        dataset[:,5] = np.random.pareto(1,taille)
        dataset[:,6] = np.random.pareto(1,taille)
        dataset[:,7] = np.random.pareto(1,taille)
        dataset[:,8] = np.random.pareto(1,taille)
        dataset[:,9] = np.random.pareto(1,taille)
        dataset[:,10] = np.random.pareto(1,taille)
        dataset[:,11] = np.random.pareto(1,taille)
        dataset[:,12] = np.random.pareto(1,taille)
        dataset[:,13] = np.random.pareto(1,taille)
        dataset[:,14] = np.random.pareto(1,taille)
    return dataset
...
x = remplirdataset(100,0.15,distri='Normal',mu=0,sigma=2,shape=2,scale=10)
#x = remplirdataset(150,1.5)


vector = getVectorsFromDataset(x,dist='cosine')
